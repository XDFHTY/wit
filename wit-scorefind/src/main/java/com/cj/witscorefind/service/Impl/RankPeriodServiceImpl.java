...
import com.cj.witscorefind.mapper.RankPeriodMapper;
 ...
 */
@Service
public class RankPeriodServiceImpl implements RankPeriodService {

    private static final Logger log = LoggerFactory.getLogger(RankPeriodServiceImpl.class);
    ...
    @Override
    public ApiResult findRankPeriod(Pager pager) {
        //返回结果
        ...
        //获取种类， 是0：名次分段 1：前N名
        Integer type = (Integer) pager.getParameters().get("type");
        ...
        //取出名次段
        List<String> scores1 = (List<String>) pager.getParameters().get("rankList");
        ...
        if (curriculumId==0){
            ...
            //查询各班每个人总分成绩
            //查询各班每个人的总分和排名
            List<ShortlistedScore> totalScores = this.rankPeriodMapper.findTotalScores(pager);
                ...
                for (ShortlistedScore ss:totalScores){
                    ...
                    if (scores!=null && scores.size()!=0 && scores.get(0)!=null) {
                        for (int i=0;i<scores.size()-1;i++){
                            ...
                            if (type == 0){//名次分段  0-50 50-100 100-200...
                                //设置最大名次
                            ...
                            }else {//前N名  0-50 0-100 0-200...
                                //设置最大名次
                                BigDecimal b = new BigDecimal(0);
                                scorePeriod.setMaxScore(b);
                                //设置最小名次
                                BigDecimal b1 = new BigDecimal(String.valueOf(scores.get(i)));
                                scorePeriod.setMinScore(b1);
                                //编辑分数段名称
                                scorePeriod.setPeriodName("0" + "-" + scores.get(i));
                            }
                            //设置人数默认为0
                        ...
                        if (type!=0){//如果是前N名 则取出最后一条
                            ...
                            scorePeriod.setMaxScore(b);
                            //设置最小名次
                            BigDecimal b1 = new BigDecimal(String.valueOf(scores.get(scores.size()-1)));
                            scorePeriod.setMinScore(b1);
                            //编辑分数段名称
                            scorePeriod.setPeriodName("0" + "-" + scores.get(scores.size()-1));
                            //设置人数默认为0
                            scorePeriod.setPeriodNum(0);
                            //添加至集合
                            shortlisted.getScorePeriods().add(scorePeriod);
                    ...
                    if (studentScoreList!=null && studentScoreList.size()!=0 && studentScoreList.get(0)!=null){
                        for (StudentScore studentScore:studentScoreList){
                            //获取名次
                            String rank = studentScore.getGradeRanking();
                            BigDecimal score = new BigDecimal(rank);
                            //获取名次段集合
                                ...
                                if (res==0){//相等 则属于这个名次段
                                    ...
                                    for (ScorePeriod sp : scorePeriods) {
                                        ...
                                        if (a == 1) {
                                            ...
                                            if (a1 == 0 || a1 == -1) {
                                                //名次 小于 最大名次 且 大于等于 最小名次  就认定为属于这个名次段
                                                // 人数+1
            ...
            //查询各班单学科每个人的成绩
            List<ShortlistedScore> subScore = this.rankPeriodMapper.findScores(pager);
            //遍历查询出来的分数信息
                ...
                for (ShortlistedScore ss:subScore){
                    ...
                    if (scores!=null && scores.size()!=0 && scores.get(0)!=null) {
                        ...
                        if (type!=0){
                            ...
                            scorePeriod.setMinScore(b1);
                            //编辑分数段名称
                            scorePeriod.setPeriodName("0" + "-" + b1);
                            //设置人数默认为0
                            scorePeriod.setPeriodNum(0);
                            //添加至集合
                            shortlisted.getScorePeriods().add(scorePeriod);
     ...
     */
    @Override
    public ApiResult rankPeriodExport(HttpServletResponse response, Pager pager, HttpServletRequest request) {
        //获取数据
        ApiResult apiResult = findRankPeriod(pager);
        ...
        try {
            ...
            try {
                ...
                String fileName = "名次分段统计";
                exportExcelUtil.exportExcel2(wb,0,"名次分段统计",style2,headers,dataHandler,response,fileName);
            } catch (Exception e) {
                log.error("导出失败");
                e.printStackTrace();
            }
